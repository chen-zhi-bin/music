package com.chen.module_login.activity
...
import com.chen.module_login.databinding.LoginActivityBinding
import com.chen.module_login.fragment.LoginFragment
...
//                mBinding.tvAgreement.setText(R.string.common_website)
    ...
    private fun onLoginSuccess(){
        if (!TextUtils.isEmpty(path)) {
            when(path){
                RoutePath.Ucenter.FRAGMENT_UCENTER -> EventBusUtils.postEvent(
                    StringEvent(
                    StringEvent.Event.SWITCH_UCENTER)
                )
