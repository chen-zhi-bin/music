package com.chen.module_login.view
...
import com.chen.module_base.ktx.gone
import com.chen.module_base.ktx.visible
import com.chen.module_base.utils.UIUtils
import com.chen.module_login.R
import com.chen.module_login.databinding.LoginCodeLayoutBinding


class CodeEditView : LinearLayout, View.OnFocusChangeListener {

    private var codeUrl = "http://10.0.2.2:8090/user/captcha?captcha_key="  //后面的随机数位数要求>13位
    private var key=""
    ...
    fun initTuringCode() {
        ...
        key = ""+(10000000000000..99999999999999).random()
        Glide.with(context).load(codeUrl + key).apply(options).into(binding.ivTuring)
    }
    fun getKey():String{
        return key
    }
