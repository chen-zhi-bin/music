...
class RetrofitFactory private constructor() {
    ...
    companion object {
        const val USER_TOKEN = "sob_token"
    ...
    private fun initCommonInterceptor(): Interceptor {
        return Interceptor { chain ->
            ...
            val sobToken = SpUtils.getString(USER_TOKEN,"")
            if(!TextUtils.isEmpty(sobToken)){
                builder.addHeader(USER_TOKEN, sobToken!!)
            }
            val response = chain.proceed(builder.build())
            ...
            for (i in 0 until responseHeadersLength) {
                ...
                if(headerName == USER_TOKEN && !TextUtils.isEmpty(headerValue)){
                    SpUtils.putString(USER_TOKEN, headerValue!!)
                    break
                }
            }
            response
